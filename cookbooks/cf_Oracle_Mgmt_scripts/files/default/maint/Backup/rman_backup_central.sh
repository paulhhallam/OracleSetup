#!/bin/bash
#######################################################################
#       Script to run daily RMAN backups to disk for Central database.
#       Run from oracle crontab daily at 2am
#       Check /u01/maint/logs/rmanlogs/rman_central_backup_yyyy_mm_dd.log for issues.
...
rman target / cmdfile=${SCRIPT_DIR}/rman_backup.rcv log=${LOG_DIR}/rmancentralfull.log

cat ${LOG_DIR}/rmancentralfull.log

# Check if errors in RMAN run.
RMANLOG=${LOG_DIR}/rmancentralfull.log
ERRCOUNT=`grep RMAN- ${RMANLOG}|grep -v RMAN-08138|grep -v RMAN-08137|wc -l`
if [ "$ERRCOUNT" -ne 0 ]; then
        echo '*****************************************************************'
        echo '************** ERRORS IN THIS RMAN RUN - CHECK LOGS *************'
        echo '*****************************************************************'
        echo 'RMAN- errors detected in log file on ' $HOSTNAME | mail -s "Central Daily RMAN backup - *** Error ***" -a ${RMANLOG} $MAIL_RECIPIENT
  else
...
rm ${LOG_DIR}/rmancentralfull.log
