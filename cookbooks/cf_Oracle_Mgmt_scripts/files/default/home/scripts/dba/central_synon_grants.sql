...
DECLARE
    ...
    target_schema VARCHAR2(20) := 'VCG_CENTRAL_APP';
...
BEGIN
        BEGIN
            EXECUTE IMMEDIATE 'GRANT DELETE ON '||src_schema|| '.SEC_PAYMENT_PASSWORDS to '|| target_schema;
            EXECUTE IMMEDIATE 'GRANT DELETE ON '||src_schema|| '.SEC_PAYMENT_PASSWORDS_PENDING to '|| target_schema;
        EXCEPTION
            WHEN OTHERS THEN
              dbms_output.Put_line('ERROR WHILE GRANTING PERMISSIONS FOR SEC_PAYMENT_PASSWORDS ');
              dbms_output.Put_line(SQLERRM);
        END;
END;
/
