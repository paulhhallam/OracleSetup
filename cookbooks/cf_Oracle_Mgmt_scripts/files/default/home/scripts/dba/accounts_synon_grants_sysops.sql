...
DECLARE
    src_schema    VARCHAR2(20) := 'CF_ACC';
    target_schema VARCHAR2(20) := 'MGAIDMA_AC';
...
BEGIN
    FOR next_row IN src_procs LOOP
        BEGIN
            EXECUTE IMMEDIATE 'GRANT SELECT ON '||src_schema|| '.' ||next_row.proc_name|| ' to '|| target_schema;
        EXCEPTION
            WHEN OTHERS THEN
...
/*--------------- CREATING THE SYNONYMS FOR JAMES ------------- */

DECLARE
    src_schema    VARCHAR2(20) := 'CF_ACC';
    target_schema VARCHAR2(20) := 'HOWLEJA_AC';
    CURSOR src_objects IS
      SELECT object_name AS object_name FROM dba_objects WHERE owner = src_schema and object_type in ('TABLE','VIEW','FUNCTION','PROCEDURE','PACKAGE','SEQUENCE');
BEGIN
    FOR next_row IN src_objects LOOP
