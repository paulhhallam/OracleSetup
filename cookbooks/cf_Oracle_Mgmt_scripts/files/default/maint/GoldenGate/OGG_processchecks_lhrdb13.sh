...
# For LHRDB13 the checks are run every 5 mminutes.
...
#           Remove the file fals as it is no longer needed
...
case "$1" in
   accounts)
        ...
        check_extr LH13ACRP /backup/oracle/OGG/accounts accounts
        check_extr AMSACDP  /backup/oracle/OGG/accounts accounts
        check_extr LH13ACEX /backup/oracle/OGG/accounts accounts
   ;;
   central)
        echo "central"
        check_extr mgr      /backup/oracle/OGG/central  central
        check_extr LH13CNRP /backup/oracle/OGG/central  central
        check_extr AMSCNDP  /backup/oracle/OGG/central  central
        check_extr LH13CNEX /backup/oracle/OGG/central  central
   ;;
   endpoint)
        echo "endpoint"
        check_extr mgr      /backup/oracle/OGG/endpoint endpoint
        check_extr LH13EPRP /backup/oracle/OGG/endpoint endpoint
        check_extr AMSEPDP  /backup/oracle/OGG/endpoint endpoint
        check_extr LH13EPEX /backup/oracle/OGG/endpoint endpoint
   ;;
   *)
