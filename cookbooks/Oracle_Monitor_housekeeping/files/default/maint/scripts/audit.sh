...
'DPA_AC1','DPA_AC2','CF_ACC','CF_ACC_APP','GGS_ADMIN','MONITOR_USER')
...
from dba_audit_trail
where username not in (
'VCG_CENTRAL_APP','DPA_CN2','DPA_CN1','GGS_ADMIN','MONITOR_USER')
and timestamp > systimestamp-numtodsinterval(902,'SECOND')
order by timestamp desc;
spool off;
EOF
...
from dba_audit_trail
where username not in (
'DPA_EP1','DPA_EP2','VCG_ENDPOINT_APP','GGS_ADMIN','MONITOR_USER')
and timestamp > systimestamp-numtodsinterval(902,'SECOND')
order by timestamp desc;
spool off;
EOF
